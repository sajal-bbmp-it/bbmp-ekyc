...
const useEKYC = (userConfig = {}) => {
    const config = Object.assign(Object.assign({}, DEFAULT_OPTIONS), userConfig);
    ...
    const [encDecValue, setEncDecValue] = (0, react_1.useState)('');
    const [loader, setLoader] = (0, react_1.useState)(false);
    const [remoteUrl, setRemoteUrl] = (0, react_1.useState)('');
    const handleSubmit = async () => {
        setLoader(true);
        const transactionNumber = (0, uuid_1.v4)();
        ...
        try {
            ...
            const encryptionResponse = await fetch(config.ENCRYPT_RESPONSE_URL, {
                method: 'POST',
                headers: {
            ...
            if (!encryptionResponse.ok) {
                ...
                console.error('Encryption API Error:', encryptionResponse.status, errorText);
                react_native_1.Alert.alert('Encryption Error', `Failed to encrypt request (${encryptionResponse.status}): ${errorText}`);
                setLoader(false);
                return;
            }
            ...
            try {
                ...
                react_native_1.Alert.alert('Server Error', 'Unexpected response format from encryption API.');
                setLoader(false);
                return;
            }
            const encDecValue = encryptionResult === null || encryptionResult === void 0 ? void 0 : encryptionResult.ENC_DEC_VALUE;
            if (!encDecValue) {
                react_native_1.Alert.alert('Encryption Error', 'encDecValue not returned from encryption API');
                setLoader(false);
                return;
            }
      ...
      try {
        ...
        window.ReactNativeWebView.postMessage("ERROR: Unable to read iframe content");
          ...
          `;
            ...
            react_native_1.Alert.alert('API Error', err instanceof Error ? err.message : 'An unexpected error occurred.');
        }
        finally {
            setLoader(false);
        }
